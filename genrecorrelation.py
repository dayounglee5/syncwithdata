...
tv_genre_song_probabilities = {
    ...
    "Comedy/Sitcom": {"Pop": 0.2046 + 0.2158, "Rock": 0.5796},  # Merged Pop/Pop into Pop
    "Crime/Thriller": {"Pop": 0.3903 + 0.4773, "Rock": 0.0313},  # Merged Pop/Pop into Pop
    "Drama": {"Pop": 0.459, "Rock": 0.1202 + 0.2901},  # Merged Rock/Rock into Rock
    "Fantasy": {"Indie Rock": 0.6544},
    "Horror": {"Pop": 0.5982, "Rock": 0.3674, "Electronica": 0.1010},
    "Reality-TV": {"Pop": 1.0},
    "Romance": {"R&B/Hip-Hop": 0.2527, "Rock": 0.3484},
    "Sci-Fi": {"Pop": 0.0856, "Rock": 0.4330 + 0.4814}  # Merged Rock/Rock into Rock
}

def normalize_probabilities(prob_dict):
    total = sum(prob_dict.values())
    if total == 0:
        return {key: 1.0 / len(prob_dict) for key in prob_dict}  # Even distribution if total is 0
    return {key: value / total for key, value in prob_dict.items()}  # Normalize
